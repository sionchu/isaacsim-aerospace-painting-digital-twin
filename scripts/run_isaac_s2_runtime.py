    ...
    try:
        ...
        for frame in range(total_frames):
            ...
            if capture is not None:
                if frame % 4 == 0 or frame == total_frames - 1:
        ...
        frame_paths = sorted(FRAME_DIR.rglob("rgb_*.png"))
        media = _encode_media(frame_paths) if not args.no_media else {"hero": None, "deposition": None, "video": None}
        def summary(values):
            return {
        ...
        if capture is not None and FRAME_DIR.exists():
            shutil.rmtree(FRAME_DIR)
        metrics = {
            ...
            "ledger": {**runtime.as_dict(), **accumulator.as_dict(), "surface_map_closure_error_kg": surface_grid.integrated_mass_kg - runtime.deposited_mass_kg},
        ...
        (RESULT_DIR / "runtime_metrics.json").write_text(json.dumps(metrics, indent=2, sort_keys=True) + "\n", encoding="utf-8")
